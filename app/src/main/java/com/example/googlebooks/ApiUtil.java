...
public class ApiUtil {
    ...
    public static final String API_KEY = "AIzaSyB2GuBaSBGDeQEoYZWs99ScMAxROndwHj8";
    ...
    //CONSTRUCTION OF THE URL THAT WE WILL USE
    ...
    public static String getJson(URL url) throws IOException{
        ...
        try{
            InputStream stream = connection.getInputStream();//Read the data
            Scanner scan = new Scanner(stream);//read the stream
            scan.useDelimiter("\\A");

            boolean hasData = scan.hasNext(); //verify if we have data
            if(hasData){
                return scan.next();
            }
            else{
    ...
    public static ArrayList<Book> getBooksFromJson(String json){
        ...
        try{
            JSONObject jsonBooks = new JSONObject(json);//CREATION OF AN JSON OBJECT
            JSONArray arrayBooks = jsonBooks.getJSONArray(ITEMS);// RECUPERATION DU TABLEAU DE LIVRES
            int numberBooks = arrayBooks.length();
